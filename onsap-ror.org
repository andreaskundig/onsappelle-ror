...
* debug
write 'debugger' in the code to set a breakpoint
...
8.1.1
requires upgrading ruby from 3.3.0 to 3.3.6
rvm install 3.3.6
rvm use 3.3.6 --default
rvm --default ruby-3.3.6
...
#+end_src
** go back to sqlite
revert original commit, keeping onsap-ror.org as it is.

#+begin_src sh :results code
(bin/rails test) 2>&1 | head -2
#+end_src
Missing a gem it depends on? can't activate sqlite3 (>= 2.1), already activated sqlite3-1.6.9
...
then use it to populate [[file:.kamal/secrets::KAMAL_REGISTRY_PASSWORD=$(rails credentials:fetch kamal.registry_password)][secrets]] 
